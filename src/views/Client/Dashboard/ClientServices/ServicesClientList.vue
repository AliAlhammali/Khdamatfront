...
export default {
  ...
  async mounted() {
    ...
    //   ).id;
    // }
    // await this.getCategoriesClient(this.paramsCategories);
    await this.getMainCategoriesClient({ "filter[isParent]": 1, listing: 1 });

    await this.getServicesClient(this.params);
  },
  computed: {
    ...mapState(useServicesClientStore, ["records", "uiFlags"]),
  ...
  methods: {
    ...
    async search(text) {
      ...
      await this.getServicesClient(key);
    },
  },
  watch: {
    $route(to, from) {
      ...
      this.getServicesClient(this.params);
    },
    filterMainCategory() {
      this.filterCategory = null;
    },
