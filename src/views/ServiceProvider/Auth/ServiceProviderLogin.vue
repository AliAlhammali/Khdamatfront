...
export default {
  ...
  computed: {
    ...mapState(useAuthServiceProviderStore, ["isLoading"]),
  },
  methods: {
    ...mapActions(useAuthServiceProviderStore, ["loginServiceProvider"]),
    async login() {
      this.v$.user.$touch();
      if (this.v$.user.$error) return;
      const isLogin = await this.loginServiceProvider(this.user);
      if (!isLogin) {
