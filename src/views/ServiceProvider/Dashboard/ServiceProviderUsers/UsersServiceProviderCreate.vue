...
export default {
  ...
  async mounted() {
    if (this.isEditDataObj) {
      ...
      await this.showUsersServiceProvider(id);
      this.dataObj = { ...this.record };
    }
  },
  computed: {
    ...mapState(useUsersServiceProviderStore, ["uiFlags", "record"]),
    isEditDataObj() {
  ...
  methods: {
    ...mapActions(useUsersServiceProviderStore, [
      "createUsersServiceProvider",
      "showUsersServiceProvider",
    ...
    actionBtn() {
      ...
      if (this.isEditDataObj) {
        this.updateUsersServiceProvider({ ...this.dataObj });
        return;
      } else {
